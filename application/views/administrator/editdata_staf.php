...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid ml-2">
            ...
            <form method="POST" action="<?php echo base_url('admin/updateDataUser/') ?><?php echo $user->kode ?>">
                <div class="row py-2">
                    <div class="col-lg-6">
                        <div class="form-group">
                            <input type="hidden" class="form-control" name="type" value="staf">
                            <input type="hidden" class="form-control" id="kode" name="kode" value="<?php echo $user->kode ?>">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="tgl_lahir" name="tgl_lahir" value="<?php echo $user->tgl_lahir; ?>">
                        </div>
                        <div class="form-group">
                            <label for="alamat" class="ml-1">Alamat</label>
                            <input type="text" class="form-control" id="alamat" name="alamat" value="<?php echo $user->alamat ?>" autocomplete="off">
                        </div>
                    </div>
                    <div class="col-lg-5 ml-2">
                        <div class="form-group">
                            <label for="jenis_kel" class="ml-1">Jenis Kelamin</label>
                            <input type="text" class="form-control" id="jenis_kel" name="jenis_kel" value="<?php echo $user->jenis_kel ?>" autocomplete="off" readonly>
                        </div>
                        <div class="form-group">
                            <label for="email" class="ml-1">Email</label>
                            <input type="text" class="form-control" id="email" name="email" value="<?php echo $user->email ?>" autocomplete="off">
                        </div>
                        <div class="form-group">
                            <label for="level_id" class="ml-1">Level</label>
                            <select name="level_id" id="level_id" class="form-control" readonly> 
                                <?php foreach ($level as $key => $value) : ?>
                                    <option value="<?php echo $value['id'] ?>" <?= $user->level_id == $value['id'] ? 'selected' : '' ?> ><?php echo $value['level'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
